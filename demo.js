//@ts-ignore
var CJDemo = ('object' === typeof module && exports) || {};
	...
	async function main() {
		...
		async function generateDenominations() {
			...
			for (let data of denominable) {
				...
				let txInfo;
				{
					let utxosRpc = await rpc.getAddressUtxos({
				...
				console.log('[DEBUG] total, txInfo', total, txInfo);
		...
		async function getCollateralTx() {
			...
			let collateralTxInfo;
			{
				...
				let output;
				let leftover = barelyEnoughest.satoshis - COLLATERAL;
				if (leftover >= COLLATERAL) {
