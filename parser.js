//@ts-ignore
var CJParser = ('object' === typeof module && exports) || {};
(function (window, CJParser) {
	'use strict';
	...
	CJParser.HEADER_SIZE = 24;
	CJParser.DSSU_SIZE = 16;
	...
	CJParser.parseDssu = function (bytes) {
		...
		// console.log(buffer.toString('utf8'));
		if (bytes.length !== CJParser.DSSU_SIZE) {
		 ...
		 */
		const SIZES = {
			SESSION_ID: CJParser.SESSION_ID_SIZE,
			STATE: 4,
			ENTRIES_COUNT: 4,
			STATUS_UPDATE: 4,
			MESSAGE_ID: 4,
		...
		let dssuMessage = {
			...
			state_id: state_id,
			state: CJParser._DSSU_STATES[state_id],
			// entries_count: 0,
			status_id: status_id,
			status: CJParser._DSSU_STATUSES[status_id],
			message_id: message_id,
			message: CJParser._DSSU_MESSAGE_IDS[message_id],
		};
	 ...
	 * @param {Uint8Array} bytes
	 */
	CJParser.parseDsf = function (bytes) {
		// console.log(
		// 	new Date(),
		...
		let offset = 0;
		let sessionId = bytes.subarray(offset, CJParser.SESSION_ID_SIZE);
		let session_id = DashTx.utils.bytesToHex(sessionId);
		offset += CJParser.SESSION_ID_SIZE;
	...
	// @ts-ignore
	window.CJParser = CJParser;
})(('object' === typeof window && window) || {}, CJParser);
if ('object' === typeof module) {
	module.exports = CJParser;
}
