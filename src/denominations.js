...
Lib.SelectDenominatedAmounts = function(nValueMax,wallet){
	...
	return {valid: nValueTotal >= CCoinJoin.GetSmallestDenomination(), setAmountsRet };
};
...
Lib.GetSpendableTXs = function(wallet) {
	...
	for(let outpoint of Lib.getWalletUTXOs(wallet)) {
		const jt = mapWallet.find(outpoint.hash);
		if(jt != mapWallet.end()) {
...
Lib.AvailableCoins = function(
	...
	for(let pcoin of Lib.GetSpendableTXs(wallet)) {
		...
		for(let i = 0; i < pcoin.tx.vout.size(); i++) {
			...
			if(Lib.IsLockedCoin(wtxid, i) && nCoinType != CoinType.ONLY_MASTERNODE_COLLATERAL) {
				continue;
			}
			...
			let solvable = provider ? Lib.IsSolvable(provider, pcoin.tx.vout[i].scriptPubKey) : false;
			let spendable = ((mine & ISMINE_SPENDABLE) != ISMINE_NO) || (((mine & ISMINE_WATCH_ONLY) != ISMINE_NO) && (coinControl && coinControl.fAllowWatchOnly && solvable));
