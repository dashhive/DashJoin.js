...
Bootstrap.save_denominated_tx = async function (
	from,
	to,
	txid,
	amount,
	...
	await mdb.paged_store(to, 'denomtx', [{ from, txid, amount, address }]);
};
						...
						for (const address of addresses) {
							...
							for (let i = 0; i < 2; i++) {
								...
								if (out.length) {
									...
									console.log(`Saved ${txid} for user: ${context.to}`);
								}
								if (err.length) {
									console.error(err);
								}
