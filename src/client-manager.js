...
Lib.ProcessMessage = function (
	...
	if (
		msg_type == Lib.NetMsgType.DSSTATUSUPDATE ||
...
Lib.GetSessionDenoms = function () {
	...
	for (const session of Lib.deqSessions.contents) {
		strSessionDenoms += Lib.CCoinJoin.DenominationToString(
			session.nSessionDenom,
		);
		strSessionDenoms += '; ';
	}
...
Lib.DoAutomaticDenominating = function (mempool, connman, fDryRun) {
	...
	let nMnCountEnabled = Lib.deterministicMNManager
		.GetListAtChainTip()
		.GetValidMNsCount();
	...
	for (const session of Lib.deqSessions.contents) {
		...
		if (Lib.WaitForAnotherBlock()) {
			...
			Lib.LogPrint(
				`CCoinJoinClientManager::DoAutomaticDenominating -- ${Lib.strAutoDenomResult.original}`,
			);
			return false;
		}
...
Lib.GetRandomNotUsedMasternode = function () {
	...
	Lib.LogPrint(
		`CCoinJoinClientManager::${__FUNCTION__} -- ${nCountEnabled} ` +
	...
	for (const dmn of vpMasternodesShuffled) {
		...
		Lib.LogPrint(
			`CCoinJoinClientManager::${__FUNCTION__} -- found, masternode=${dmn.collateralOutpoint.ToStringShort()}`,
		); // TODO: FIXME: do dmn.collateralOutpoint
		return dmn;
	}
...
Lib.ProcessPendingDsaRequest = function (connman) {
	for (const session of Lib.deqSessions.contents) {
		if (session.ProcessPendingDsaRequest(connman)) {
			// TODO: FIXME: must implement ProcessPendingDsaRequest
...
Lib.TrySubmitDenominate = function (mnAddr, connman) {
	for (const session of Lib.deqSessions.contents) {
		...
		if (
			mnMixing &&
			mnMixing.pdmnState.addr == mnAddr &&
...
Lib.MarkAlreadyJoinedQueueAsTried = function (dsq) {
	for (const session of Lib.deqSessions.contents) {
		...
		let mnMixing = null;
		mnMixing = session.GetMixingMasternodeInfo(mnMixing);
		if (mnMixing && mnMixing.collateralOutpoint == dsq.masternodeOutpoint) {
			// TODO: FIXME: need dsq.masternodeOutpoint to be coded
...
Lib.UpdatedBlockTip = function (pindex) {
	...
	Lib.LogPrint(
		`CCoinJoinClientManager::UpdatedBlockTip -- nCachedBlockHeight: ${Lib.nCachedBlockHeight}`,
	);
};
...
Lib.DoMaintenance = function (mempool, connman) {
	...
	if (nDoAutoNextRun == nTick) {
		...
		nDoAutoNextRun =
			...
			GetRandInt(COINJOIN_AUTO_TIMEOUT_MAX - COINJOIN_AUTO_TIMEOUT_MIN);
	}
};
